...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<void> initProfile() async {
    try {
      ...
      if (data.isNotEmpty) {
        ...
        AppLogger.debug(
          "Profile loaded from local storage: ${_userProfile?.name}",
        );
  ...
  Future<void> fetchProfile() async {
    ...
    try {
      ...
      if (error.toString().contains('PGRST116') ||
          error.toString().contains('multiple (or no) rows returned')) {
        _userProfile = null;
  ...
  void setProfile(UsersModel profile) async {
    final id = Supabase.instance.client.auth.currentUser?.id;
    if (id == null) {
  ...
  Future<void> autoFillProfile() async {
    ...
    AppLogger.debug(
      "Auto-filled profile data: ${cityController.text}, ${stateController.text}, $latitude, $longitude, $locationiqPlaceId, $postalCode",
    );
  ...
  Future<void> setLanguagePreference(String language) async {
    ...
    status = "Saving language preference...";
    error = null;
    notifyListeners();
    ...
    try {
      ...
      final request = http.MultipartRequest(
        'POST',
        Uri.parse('${ApiManager.baseUrl}/user/language'),
      );
      request.headers.addAll({
        'Authorization': 'Bearer ${session!.accessToken}',
        // ngrok tunnel page skip
      ...
      if (response.statusCode == 200) {
        ...
        if (_userProfile != null) {
          _userProfile = _userProfile!.copyWith(prefered_language: language);
        ...
        status = "Language preference saved successfully";
        AppLogger.debug("Language preference updated to: $language");
        notifyListeners();
        ...
        error = "Failed to save language preference: ${response.body}";
        AppLogger.error("Failed to save language: ${response.body}");
        status = "";
        notifyListeners();
      }
    } catch (e) {
      error = "Error saving language preference: $e";
